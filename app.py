...
<nav class="navbar fixed-top navbar-expand-lg navbar-dark" style="background-color: #3498DB;">
  ...
  <div class="collapse navbar-collapse" id="navbarNav">
    <ul class="navbar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link" href="https://www.linkedin.com/in/muhammad-ghulam-jillani-38603820a/" target="_blank">LinkedIn</a>
      </li>
    </ul>
  </div>
</nav>
...
# Make Function for floating Values
def Rounding_Value(input_val):
    if input_val.values > 1:
        val=float(round(input_val,2))
    else:
        val=float(round(input_val,8))
...
col1_price = Rounding_Value(col1_df.weightedAvgPrice)
col2_price = Rounding_Value(col2_df.weightedAvgPrice)
col3_price = Rounding_Value(col3_df.weightedAvgPrice)
col4_price = Rounding_Value(col4_df.weightedAvgPrice)
col5_price = Rounding_Value(col5_df.weightedAvgPrice)
col6_price = Rounding_Value(col6_df.weightedAvgPrice)
col7_price = Rounding_Value(col7_df.weightedAvgPrice)
col8_price = Rounding_Value(col8_df.weightedAvgPrice)
col9_price = Rounding_Value(col9_df.weightedAvgPrice)
...
st.info('Credit: Created by Mr : M.G.Jillani Soft Tech (aka [Data Scientist](https://www.linkedin.com/in/muhammad-ghulam-jillani-38603820a/))')
